...
FIXTURE_PATH = REPO_ROOT / "sample_ai_session.jsonl"
...
class TestSampleFixture:
    ...
    def test_sessions_present(self):
        ...
        assert "demo-session-01" in session_ids
        assert "demo-session-02" in session_ids

    def test_model_populated_on_assistant_rows(self):
        for r in self.by_type.get("assistant_response", []):
            assert "model" in r  # model key must exist; may be empty if fixture omits it
